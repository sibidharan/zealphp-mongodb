...
$zClient = new ZealPHP\MongoDB\Client($uri);
...
function compare($label, $z, $m) {
    ...
    if ($m instanceof MongoDB\Model\BSONDocument || $m instanceof MongoDB\Model\BSONArray) {
        $m = json_decode(json_encode($m), true);
    }
    if (is_array($m)) {
        array_walk_recursive($m, function(&$v) {
            if ($v instanceof MongoDB\Model\BSONDocument) $v = json_decode(json_encode($v), true);
            if ($v instanceof MongoDB\Model\BSONArray) $v = json_decode(json_encode($v), true);
            if ($v instanceof MongoDB\BSON\ObjectId) $v = (string)$v;
            if ($v instanceof MongoDB\BSON\UTCDateTime) $v = $v->toDateTime()->getTimestamp();
        });
    }
    if ($z === $m) {
        echo "  ✓ $label\n"; $passed++;
